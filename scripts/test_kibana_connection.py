...
async def test_connection(config_path="config.yaml"):
    ...
    try:
        ...
        if not auth_cookie:
            logger.error(
                "No authentication cookie provided. Set KIBANA_AUTH_COOKIE environment variable."
            )
            return False
        ...
        logger.info(
            "Creating HTTP client with SSL verification disabled and 60s timeout"
        )
        async with httpx.AsyncClient(verify=False, timeout=60.0) as client:
            ...
            headers = {"kbn-version": "7.10.2", "Content-Type": "application/json"}
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if "saved_objects" in patterns:
                        logger.info(
                            f"Found {len(patterns['saved_objects'])} index patterns"
                        )
                    ...
                    logger.error(
                        f"Failed to connect to Kibana API: {response.status_code}"
                    )
                ...
                if response.status_code == 200:
                    ...
                    for index in indices:
                        if "kibana" in index.get("index", ""):
                            logger.info(
                                f"  - {index.get('index')}: {index.get('docs.count')} docs"
                            )
                else:
                    logger.warning(
                        f"Could not access Elasticsearch directly: {response.status_code}"
                    )
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if "fields" in result:
                        ...
                        if timestamp_fields:
                            logger.info(
                                f"Found timestamp fields: {', '.join(timestamp_fields[:5])}"
                            )
                        if level_fields:
                            logger.info(
                                f"Found log level fields: {', '.join(level_fields[:5])}"
                            )
                        if message_fields:
                            logger.info(
                                f"Found message fields: {', '.join(message_fields[:5])}"
                            )
                else:
                    logger.warning(
                        f"Failed to get index fields: {response.status_code}"
                    )
            except Exception as e:
            ...
            for index_pattern in index_patterns:
                ...
                try:
                    ...
                    logger.info(
                        f"Search response status for {index_pattern}: {response.status_code}"
                    )
                    ...
                    if response.status_code == 200:
                        ...
                        if (
                            ...
                            and "hits" in result["rawResponse"]
                            and "hits" in result["rawResponse"]["hits"]
                        ):
                            hits = result["rawResponse"]["hits"]["hits"]
                            logger.info(
                                f"Retrieved {len(hits)} logs successfully from {index_pattern}!"
                            )
                            # Display a sample log
                            if hits:
                                logger.info(
                                    f"Sample log from {index_pattern}: {json.dumps(hits[0]['_source'], indent=2)}"
                                )
                            ...
                            logger.warning(
                                f"No logs found in indices matching {index_pattern}"
                            )
                    else:
                        logger.warning(
                            f"Failed to retrieve logs from {index_pattern}: {response.status_code}"
                        )
                        logger.warning(f"Response body: {response.text}")
                except Exception as e:
                    logger.warning(
                        f"Exception during log retrieval from {index_pattern}: {e}"
                    )

            if not success:
                logger.error(
                    "Could not retrieve logs from any of the attempted index patterns."
                )
                return False
