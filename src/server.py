...
kibana_max_discover_indexes = es_config["max_discover_index"]
...
async def discover_indexes(ctx: Context = None) -> Dict:
    ...
    try:
        ...
        max_discover_indexes = get_config_value(
            "elasticsearch.kibana_api.max_discover_indexes", kibana_max_discover_indexes
        )
        # Create a new client for this request
        async with get_http_client() as client:
            # First try using Kibana's index pattern API
            url = f"https://{host}{base_path}/api/saved_objects/_find?per_page={max_discover_indexes}&type=index-pattern"
...
async def get_ai_rules(ctx: Context = None) -> Dict:
    ...
    try:
        # Get directory of the current script
        rule_file_path = "docs/AI_RULES.md"
        script_dir = os.path.dirname(os.path.abspath(__file__))
        project_root = os.path.dirname(script_dir)  # Go up one level to project root
        rules_path = os.path.join(project_root, rule_file_path)
        ...
        if os.path.exists(rules_path):
            ...
            return {
                ...
                "rules": "File not found: " + rule_file_path,
            }
    except Exception as e:
...
if __name__ == "__main__":
    ...
    if transport == "http":
        ...
        async def api_set_config(request_data: dict):
            """Set one or more configuration values dynamically."""
            configs_to_set = request_data.get("configs_to_set")
            if not configs_to_set:
                return {"success": False, "error": "configs_to_set is required"}
            return await set_config(configs_to_set)
